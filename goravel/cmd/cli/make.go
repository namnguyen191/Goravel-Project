	...
	case "migration":
		{
			...
			if runtime.GOOS == "windows" {
				upFile = grv.RootPath + "\\migrations\\" + fileName + "." + dbType + ".up.sql"
				downFile = grv.RootPath + "\\migrations\\" + fileName + "." + dbType + ".down.sql"
			} else {
				upFile = grv.RootPath + "/migrations/" + fileName + "." + dbType + ".up.sql"
				downFile = grv.RootPath + "/migrations/" + fileName + "." + dbType + ".down.sql"
			}
	...
	case "handler":
		{
			...
			fileName := grv.RootPath + "/handlers/" + strings.ToLower(arg3) + ".go"
	...
	case "model":
		{
			...
			fileName := grv.RootPath + "/data/" + strings.ToLower(modelName) + ".go"
			if fileExist(fileName) {
				exitGracefully(errors.New(fileName + " already exists!"))
			}
	...
	case "mail":
		{
			...
			htmlMail := grv.RootPath + "/mail/" + strings.ToLower(arg3) + ".html.tmpl"
			plainMail := grv.RootPath + "/mail/" + strings.ToLower(arg3) + ".plain.tmpl"
