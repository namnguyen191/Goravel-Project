...
import (
	...
	apimaildriver "github.com/ainsleyclark/go-mail/drivers"
...
func (m *Mail) SendUsingAPI(msg Message, transport string) error {
	...
	switch transport {
	case "mailgun":
		driver, err = apimaildriver.NewMailgun(cfg)
	case "postal":
		driver, err = apimaildriver.NewPostal(cfg)
	case "sendgrid":
		driver, err = apimaildriver.NewSendGrid(cfg)
	case "sparkpost":
		driver, err = apimaildriver.NewSparkPost(cfg)
	case "postmark":
		driver, err = apimaildriver.NewPostmark(cfg)
	default:
		driver, err = apimaildriver.NewSMTP(cfg)
	}
